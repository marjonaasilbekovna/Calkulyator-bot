...
async def callback_answer(callback: CallbackQuery):
    if callback.message.text == "|":
        if callback.data in "DC": 
            await callback.answer(f"❗ O'chirish uchun ifoda mavjud emas!", show_alert=True)
        ...
        if callback.data == "D":
            ...
            await callback.message.edit_text(callback.message.text[:-1] + callback.data + callback.message.text[-1])
            await callback.message.edit_reply_markup(reply_markup=calculator_button.calculator_builder)

        elif callback.data in "+-*/":
            await callback.message.edit_text(callback.message.text[:-2] + callback.data + "|", reply_markup=calculator_button.calculator_builder)
